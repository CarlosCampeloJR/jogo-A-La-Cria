...
def atualizar_personagem():
    ...
    for obstaculo in obstaculos:
        ...
        if obstaculo[0] + obstaculo[2] < 0:  # Usa largura do obstáculo
            obstaculos.remove(obstaculo)

        # Verifica colisão com o personagem
        if (
            (personagem_x < obstaculo[0] + obstaculo[2]) and  # Usa largura do obstáculo
            (personagem_x + 17 > obstaculo[0]) and
            (personagem_y < obstaculo[1] + obstaculo[3]) and  # Usa altura do obstáculo
...
def desenhar_obstaculos():
    for obstaculo in obstaculos:
        x, y, largura, altura = obstaculo
        px.rect(x, y, largura, altura, 6)  # Desenha o obstáculo com largura e altura específicas

# Função de atualização
def update():
    global is_game_over, multiplicador_velocidade, tela, opcao_selecionada, contador, inicio_jogo
...
def reiniciar_jogo():
    global personagem_x, personagem_y, personagem_velocidade_vertical, personagem_no_chao, obstaculos, is_game_over, multiplicador_velocidade, tela, inicio_jogo
    personagem_x = 6
    personagem_y = 133
