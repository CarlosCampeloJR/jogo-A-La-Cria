...
# Variáveis do jogo
nx = [0, 180, 360, 540, 720, 900, 1080]  # Posições horizontais do chão
ny = [150] * 7  # Posições verticais (fixas) do chão
nvx = [1] * 7  # Velocidades do chão
...
obstaculo_vx = 3  # Velocidade horizontal dos obstáculos
...
def atualizar_chao():
    ...
    for i in range(len(nx)):
        nvx[i] += 0.005  # Incrementa a velocidade gradualmente
        nx[i] = nx[i] + nvx[i]
        if nx[i] > px.width:
...
def atualizar_personagem():
    ...
    for obstaculo in obstaculos:
        obstaculo[0] -= obstaculo_vx  # Move o obstáculo para a esquerda
...
def desenhar_chao():
    for i in range(len(nx)):
        px.blt(nx[i], ny[i], 1, 0, -1, 200, 31)  # Desenha o chão
...
def reiniciar_jogo():
    global personagem_x, personagem_y, personagem_velocidade_vertical, personagem_no_chao, obstaculos, is_game_over
    personagem_x = 0  # Posição horizontal do personagem
